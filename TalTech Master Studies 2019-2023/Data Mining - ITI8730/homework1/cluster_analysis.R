library(matrixStats)
#library(cluster)
source("C:/Users/urmas/Documents/TalTech/Andmekaeve/assignment1/distances.R")
...
knneighbors <- function(data_to_classify, data, labels, k=1, metric="euclidean") {
  ...
  res <- matrix(0.0, nrow=nrow(data_to_classify), ncol=1)
...
plot_results <- function(data, centers, label_colors, centers_color="black"){
  ...
  plot(data, col=label_colors, xlim=xlim, ylim=ylim, main="k-means")
  par(new=TRUE)
  plot(centers, col=centers_color, xlim=xlim, ylim=ylim, xlab="", ylab="")
}
